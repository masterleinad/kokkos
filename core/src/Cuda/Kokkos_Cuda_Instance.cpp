...
void CudaInternal::initialize(int cuda_device, cudaStream_t stream,
  ...
  if (device_for_stream != cuda_device) {
    std::stringstream ss;
    ss << "Error: The provided stream is associated with device " << device_for_stream
       << " but device " << cuda_device << " was requested in the execution space instance constructor!";
