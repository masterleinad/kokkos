...
class ScratchMemorySpace<Kokkos::Experimental::SYCL> {
 ...
 public:
  ...
  KOKKOS_INLINE_FUNCTION void* get_shmem(const IntType& size,
                                         int level = -1) const {
	  constexpr bool alignment_requested = false;
	  if (level == 1)
    return sycl::global_ptr<void>(get_shmem_common<alignment_requested>(size, 1, m_iter_L1, m_end_L1));
	  else
		  return sycl::local_ptr<void>(get_shmem_common<alignment_requested>(size, 1, m_iter_L0, m_end_L0));
  }
  ...
  KOKKOS_INLINE_FUNCTION void* get_shmem_aligned(const IntType& size,
                                                 ...
                                                 int level = -1) const {
	  constexpr bool alignment_requested = true;
  if (level==1)
    return sycl::global_ptr<void>(get_shmem_common<alignment_requested>(size, alignment,
                                                         m_iter_L1, m_end_L1));
    else
    return sycl::local_ptr<void>(get_shmem_common<alignment_requested>(size, alignment,
                                                          m_iter_L0, m_end_L0));
  }

 private:
  template <bool alignment_requested, typename IntType, typename PointerType>
