...
KOKKOS_ENSURES(!graph_node);
 ...
 //   KOKKOS_ENSURES(graph_node.get_type() == sycl::ext::oneapi::experimental::node_type::kernel)
	...
	last_reduction_event = q.submit(cgh_lambda);
#ifndef KOKKOS_IMPL_SYCL_USE_IN_ORDER_QUEUES
    q.ext_oneapi_submit_barrier(std::vector<sycl::event>{last_reduction_event});
#endif
}
}
    ...
    if (host_result_ptr) {
      ...
      if constexpr(Policy::is_graph_kernel::value)
	      Kokkos::abort("parallel_reduce not implemented for graph kernels if result is not device-accessible!");
 ...
 public:
  void execute() const {
    ...
    std::scoped_lock<std::mutex> scratch_buffers_lock(instance.m_mutexScratchSpace);
