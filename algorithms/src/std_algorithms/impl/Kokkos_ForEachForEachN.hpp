...
#define KOKKOS_DO_PRAGMA(x) _Pragma(#x)
      ...
      #define KOKKOS_NVCC_DIAG_PRAGMA_PUSH \
        KOKKOS_DO_PRAGMA(nv_diagnostic push) 
      #define KOKKOS_NVCC_DIAG_PRAGMA_SUPPRESS(WARNING) \
        KOKKOS_DO_PRAGMA(nv_diag_suppress = WARNING)
      #define KOKKOS_NVCC_DIAG_PRAGMA_POP \
        KOKKOS_DO_PRAGMA(nv_diagnostic pop)
    #elif defined __CUDA_ARCH__
      #define KOKKOS_NVCC_DIAG_PRAGMA_PUSH \
        KOKKOS_DO_PRAGMA(diagnostic push)
	      #define KOKKOS_NVCC_DIAG_PRAGMA_SUPPRESS(WARNING) \
        KOKKOS_DO_PRAGMA(diag_suppress WARNING)
      #define KOKKOS_NVCC_DIAG_PRAGMA_POP \
        KOKKOS_DO_PRAGMA(diagnostic pop)
    #endif
  ...
  #define KOKKOS_NVCC_DIAG_PRAGMA_PUSH \
    ...
    KOKKOS_DO_PRAGMA(diag_suppress = WARNING)
      #define KOKKOS_NVCC_DIAG_PRAGMA_POP \
        KOKKOS_DO_PRAGMA(diagnostic pop)
#else
	  #define KOKKOS_NVCC_DIAG_PUSH
          #define KOKKOS_NVCC_DIAG_PRAGMA_PUSH_AND_SUPPRESS(WARNING)
      #define KOKKOS_NVCC_DIAG_PRAGMA_POP
#endif
...
KOKKOS_NVCC_DIAG_PRAGMA_PUSH
KOKKOS_NVCC_DIAG_PRAGMA_SUPPRESS(20011)
KOKKOS_NVCC_DIAG_PRAGMA_SUPPRESS(20014)
...
for_each_impl([[maybe_unused]] const char* label, const HandleType& handle,
  ...
  if constexpr (is_execution_space_v<HandleType>) {
    KOKKOS_IF_ON_DEVICE((Kokkos::abort("error");))
    ::Kokkos::parallel_for(
...
#undef KOKKOS_DO_PRAGMA
          #undef KOKKOS_NVCC_DIAG_PRAGMA_PUSH
          #undef KOKKOS_NVCC_DIAG_PRAGMA_SUPPRESS
      #undef KOKKOS_NVCC_DIAG_PRAGMA_POP
