 ...
 public:
  ...
  DualView(const Impl::ViewCtorProp<P...>& arg_prop,
    ...
    static_assert(!Impl::ViewCtorProp<P...>::has_execution_space, "DualView: Kokkos::SequentialHostInit isn't compatible with providing an execution space instance!");
      ...
      d_view = Kokkos::create_mirror_view_and_copy(typename traits::memory_space{}, h_view);
      }
	      else{
  ...
  void impl_resize(const Impl::ViewCtorProp<ViewCtorArgs...>& arg_prop,
    ...
    if constexpr (alloc_prop_input::sequential_host_init) {
          static_assert(alloc_prop_input::initialize, "DualView: Kokkos::SequentialHostInit isn't compatible with Kokkos::WithoutInitializing!");
